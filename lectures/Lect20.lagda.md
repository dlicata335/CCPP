...
```
  ...
  lookup : {low : Nat} {high : Nat} 
         ...
         → (k : Key)
         → Decision (InTree k t)
  lookup (Empty lh) lookkey = Inr {!!}
  lookup (Node x xv l r) lookkey with check< (! lookkey) (! x)
  lookup (Node (1+ x , _) xv l r) lookkey    | Equal (Refl _) = Inl Here
  lookup (Node x xv l r) lookkey    | Less lookkeyx with lookup l lookkey
  lookup (Node x xv l r) lookkey    | Less lookkeyx    | Inl inleft =  Inl (ThereLeft inleft)
  ...
  weaken : {low high high' : Nat}
         → SortedTree low high
         → high ≤ high'
         → SortedTree low high'
  weaken = {!!}

  weaken-low : {low low' high  : Nat}
         → SortedTree low high
         → low' ≤ low
         → SortedTree low' high
  weaken-low = {!!}
  ...
  insert : {low : Nat} {high : Nat} 
         ...
         → SortedTree (min (newkey minus1) low) (max (1+ (! newkey)) high)
  insert (Empty lh) newkey newv = Node newkey newv (Empty {!!}) (Empty {!!})
  insert (Node x xv l r) newkey newv with check< (! newkey) (! x)
  ...    | Equal p = Node x newv (weaken-low l {!!}) (weaken r {!!})
  ...    | Less p = Node x xv (weaken (insert l newkey newv) {!!}) (weaken r {!!})
  ...    | Greater p = {!!}
```
